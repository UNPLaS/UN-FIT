...
echo "target remote |" $QEMU " -gdb stdio -S -M "$mach "-m" $mem "-no-reboot -singlestep -d nochain,cpu -D" $ramdir/logs_qemu.txt "-kernel" $bin  > $ramdir/trace.gdb
...
case $arch in
    0)
     # nohup $qemu_arm -S -M $mach -m $mem -nographic -singlestep -d nochain,cpu -D $ramdir/logs_qemu.txt -gdb tcp::2159 -kernel $bin &
      $gdbarm -x $ramdir/trace.gdb > /dev/null 2>&1
